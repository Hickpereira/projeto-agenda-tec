...
class AgendaSystem {
  ...
  setupEventListeners() {
    ...
    document
      ...
      ?.addEventListener("click", this.increaseFontSize.bind(this));
    document
      .getElementById("fontSizeDecrease")
      ?.addEventListener("click", this.decreaseFontSize.bind(this));
    document
      .getElementById("highContrastToggle")
      ?.addEventListener("click", this.toggleHighContrast.bind(this));
    ...
    [
      "loginTabResponsavel",
      "loginTabCoordenador",
      "registerTabResponsavel",
      "registerTabCoordenador",
    ].forEach((id) => {
  ...
  showOrientadorInfo(selectedOption) {
    ...
    // Garante que selectedOption não é nulo antes de tentar ler 'value'
    const orientadorId = selectedOption ? selectedOption.value : null;

    // A condição de falha: se não houver ID ou se a 'option' não tiver os dados embutidos
    if (!orientadorId || !selectedOption.dataset.info) {
      orientadorInfo.classList.add("hidden");
      this.selectedOrientador = null; // Define como nulo, o que causa o erro de validação (correto!)
      return;
    }

    // Se passou na validação, processa e define this.selectedOrientador
    const orientadorData = JSON.parse(selectedOption.dataset.info);

    // ATRIBUIÇÃO CRÍTICA: O objeto do orientador é salvo aqui
    this.selectedOrientador = {
      id: orientadorId,
      ...orientadorData,
    };

    // Atualiza a interface com as informações do orientador
  ...
  showSchedulesList() {
    const userSchedules = this.requests.filter(
                            ...
                            : pendingRequests
                                ...
                                <div class="request-item">
                                    ...
                                    <div class="request-actions">
                                        <button class="btn btn-primary" onclick="agendaSystem.approveRequest('${
                                          request.id
                                        }')">
                                            <i class="fas fa-check"></i> Aceitar
                                        </button>
                                        <button class="btn btn-outline" onclick="agendaSystem.rejectRequest('${
                                          request.id
                                        }')">
