class AgendaSystem {
  ...
  setupEventListeners() {
    ...
    document
      ...
      ?.addEventListener("click", () => this.hideModal("loginModal"));
    document
      .getElementById("closeRegister")
      ?.addEventListener("click", () => this.hideModal("registerModal"));
    ...
    // FUNCAO MASCARA
    // Máscara para o CPF do Orientador no formulário de cadastro
    const cpfInput = document.getElementById("cpf_orientador");
    if (cpfInput) {
      cpfInput.addEventListener("input", () => maskCPF(cpfInput));
    }

    // Máscara para o Telefone do Orientador no formulário de cadastro
    const telOrientadorInput = document.getElementById("telefone_orientador");
    if (telOrientadorInput) {
      telOrientadorInput.addEventListener("input", () =>
        maskPhone(telOrientadorInput)
      );
      telOrientadorInput.setAttribute("maxlength", "15"); // (XX) XXXXX-XXXX
    }

    // Máscara para o Telefone do Responsável no formulário de cadastro
    const telResponsavelInput = document.getElementById("telefone_responsavel");
    if (telResponsavelInput) {
      telResponsavelInput.addEventListener("input", () =>
        maskPhone(telResponsavelInput)
      );
  ...
  switchAuthTab(kind, type) {
    ...
    form
      .querySelector(
        `#${kind}Tab${type === "responsavel" ? "Responsavel" : "Coordenador"}`
      )
    ...
    form
      .querySelector(
        `#${kind}Pane${type === "responsavel" ? "Responsavel" : "Coordenador"}`
      )
      ?.classList.add("active");
    ...
    if (userTypeInput) {
      userTypeInput.value =
        type === "responsavel" ? "responsavel" : "coordenador";
    }
  ...
  showScheduleModal() {
    ...
    document
      ...
      .addEventListener("submit", this.handleScheduleRequest.bind(this));

    // Event listener para seleção de orientador
    document
      .getElementById("orientadorSelect")
  ...
  showOrientadorInfo(orientadorId) {
    ...
    if (orientadorId === "coordenador") {
      this.selectedOrientador = {
        ...
        description:
          "Responsável pela orientação educacional e acompanhamento dos alunos",
      ...
      document
        .querySelector(".orientador-details")
        .appendChild(statusIndicator);
  ...
  handleScheduleRequest(e) {
    ...
    if (
      ...
      typeof window.scheduleService.createRequest === "function"
    ) {
      window.scheduleService
      ...
      this.showNotification(
        "Erro de sistema. Não foi possível enviar a solicitação.",
        "error"
      );
    }
  }
