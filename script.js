class AgendaSystem {    constructor() {
    ...
    setupEventListeners() {
        ...
        document.getElementById('closeLogin')?.addEventListener('click', () => this.hideFormModal('loginModal'));
        document.getElementById('closeRegister')?.addEventListener('click', () => this.hideFormModal('registerModal'));
    ...
    initAccessibilityPanel() {
        ...
        const applyPrefs = (prefs) => {
            ...
            if (prefs.highContrast) document.body.classList.add('high-contrast'); else document.body.classList.remove('high-contrast');
            document.documentElement.style.setProperty('--base-font-scale', `${prefs.fontScale}`);
            document.documentElement.style.fontSize = `${Math.round(prefs.fontScale * 100)}%`;
            if (fontDisplay) fontDisplay.textContent = `${Math.round(prefs.fontScale * 100)}%`;
            document.documentElement.style.setProperty('--line-scale', `${prefs.lineScale}`);
            if (lineDisplay) lineDisplay.textContent = `${Math.round(prefs.lineScale * 100)}%`;
        ...
        const loadPrefs = () => {
            try {
                ...
                const prefs = raw ? JSON.parse(raw) : { darkMode: false, highContrast: false, fontScale: 1, lineScale: 1, readableFont: false, underlineLinks: false, reduceMotion: false, focusVisible: false };
                return {
            ...
            } catch { return { darkMode: false, highContrast: false, fontScale: 1, lineScale: 1, readableFont: false, underlineLinks: false, reduceMotion: false, focusVisible: false }; }
        };
        ...
        fontMinus?.addEventListener('click', () => {
            prefs.fontScale = Math.max(0.8, Number((prefs.fontScale - 0.05).toFixed(2)));
            applyPrefs(prefs);
            savePrefs(prefs);
        });

        fontPlus?.addEventListener('click', () => {
            prefs.fontScale = Math.min(1.5, Number((prefs.fontScale + 0.05).toFixed(2)));
            applyPrefs(prefs);
            savePrefs(prefs);
        });

        lineMinus?.addEventListener('click', () => {
            prefs.lineScale = Math.max(1, Number((prefs.lineScale - 0.05).toFixed(2)));
            applyPrefs(prefs);
            savePrefs(prefs);
        });

        linePlus?.addEventListener('click', () => {
            prefs.lineScale = Math.min(2, Number((prefs.lineScale + 0.05).toFixed(2)));
            applyPrefs(prefs);
            savePrefs(prefs);
        });
        ...
        resetBtn?.addEventListener('click', () => {
            prefs = { darkMode: false, highContrast: false, fontScale: 1, lineScale: 1, readableFont: false, underlineLinks: false, reduceMotion: false, focusVisible: false };
            applyPrefs(prefs);
            savePrefs(prefs);
        });
    }
    ...
    getStoredUsers() {
        try {
            ...
            console.warn('Falha ao ler agenda_users do storage, resetando para vazio.', e);
            return [];
        }
    }
    ...
    showScheduleModal() {
        ...
        document.getElementById('closeSchedule').addEventListener('click', () => this.hideFormModal('scheduleModal'));
        ...
        // Definir data mínima como hoje (formato local sem UTC para evitar +1 dia)
        const now = new Date();
    ...
    generateOrientadorOptions() {
        return '<option value="coordenador">Orientador Educacional - Orientação Geral</option>';
    }
    ...
    // Substitui termos visíveis na interface para "Orientador Educacional"
    replaceCoordinatorTerms() {
        ...
        while ((node = walker.nextNode())) {
            ...
            replacements.forEach(([from, to]) => {
                // evitar substituir dentro de campos de formulário invisíveis; TreeWalker já ignora atributos
   ...
   handleScheduleRequest(e) {
    ...
    if (window.scheduleService && typeof window.scheduleService.createRequest === 'function') {
        window.scheduleService.createRequest(request)
            .then(() => {
                ...
                this.hideFormModal('scheduleModal');
    ...
    generateAvailableTimeSlots() {
        ...
        if (customTimeSlots.length > 0 && selectedDate) {
            ...
            const daySlots = customTimeSlots.filter(slot => slot.dayOfWeek === systemDay);
    ...
    async showSchedulesList() {
        ...
        if (this.userType === 'coordenador') {
            schedulesToShow = this.requests.filter(req => req.status === 'approved');
            modalTitle = 'Agendamentos Confirmados';
        } else {
            schedulesToShow = this.requests.filter(req => req.userId === this.currentUser.id);
                    ...
                    <div class="tab-content">
                        <div id="all-tab" class="tab-pane active">
                            <div class="schedules-list" id="schedulesList">
                                ${schedulesToShow.length === 0 ? 
                                    ...
                                    schedulesToShow.map(schedule => this.createScheduleItem(schedule)).join('')
                                }
                            </div>
                        </div>
                        ...
                        <div id="upcoming-tab" class="tab-pane">
                            <div class="schedules-list" id="upcomingSchedulesList">
                                ${this.getUpcomingSchedules(schedulesToShow).length === 0 ? 
                                    '<div class="empty-state"><i class="fas fa-calendar-plus"></i><p>Nenhum agendamento futuro.</p></div>' :
                                    this.getUpcomingSchedules(schedulesToShow).map(schedule => this.createScheduleItem(schedule)).join('')
                                }
                            </div>
                        </div>
                        ...
                        <div id="past-tab" class="tab-pane">
                            <div class="schedules-list" id="pastSchedulesList">
                                ${this.getPastSchedules(schedulesToShow).length === 0 ? 
                                    '<div class="empty-state"><i class="fas fa-calendar-minus"></i><p>Nenhum agendamento passado.</p></div>' :
                                    this.getPastSchedules(schedulesToShow).map(schedule => this.createScheduleItem(schedule)).join('')
                                }
                            </div>
                        </div>
                    </div>
                        ...
                        <div id="pending-tab" class="tab-pane active">
                            <div class="requests-list">
                                ${pendingRequests.length === 0 ? 
                                    ...
                                    pendingRequests.map(request => this.createRequestItem(request)).join('')
                                }
                            </div>
                        </div>
                        ...
                        <div id="accepted-tab" class="tab-pane">
                            <div class="requests-list">
                                ${acceptedRequests.length === 0 ? 
                                    ...
                                    acceptedRequests.map(request => this.createRequestItem(request, true)).join('')
                                }
                            </div>
                        </div>
                        ...
                        <div id="rejected-tab" class="tab-pane">
                            <div class="requests-list">
                                ${rejectedRequests.length === 0 ? 
                                    ...
                                    rejectedRequests.map(request => this.createRequestItem(request, true)).join('')
                                }
                            </div>
                        </div>
                    </div>
            ...
            <div class="request-item ${statusClass}">
                <div class="request-info">
                    ...
                    <div class="request-details">
                        ...
                        ${request.responseDate ? `<p><strong>Respondido em:</strong> ${this.formatDate(request.responseDate)}</p>` : ''}
                    </div>
                </div>
                ${!isReadOnly ? `
    ...
    switchRequestTab(tabName, modalId = 'requestsModal') {
        ...
        document.querySelectorAll(`#${modalId} .tab-pane`).forEach(pane => pane.classList.remove('active'));
        
        // Add active class to selected tab and pane
        document.querySelector(`#${modalId} [data-tab="${tabName}"]`).classList.add('active');
    ...
    showScheduleManager() {
                                                ...
                                                ${['Segunda', 'Terça', 'Quarta', 'Quinta', 'Sexta', 'Sábado', 'Domingo'].map((day, index) => `
                                                    <label class="checkbox-label">
                                                        <input type="checkbox" name="workingDays" value="${index}" 
                                                               ${this.getScheduleSettings().workingDays.includes(index) ? 'checked' : ''}>
                                                        ${day}
                                                    </label>
                                                `).join('')}
                                            </div>
        ...
        // Event listeners
        document.getElementById('closeScheduleManager').addEventListener('click', () => {
            this.hideModal('scheduleManagerModal');
            document.getElementById('scheduleManagerModal').remove();
        });
        
        // Tab switching
        document.querySelectorAll('#scheduleManagerModal .tab-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                const buttonEl = e.currentTarget || e.target.closest('button');
                const tabName = buttonEl?.dataset.tab;
                        ...
                        <div class="reports-grid">
                            <div class="report-card">
                                ...
                                <div class="stats-grid">
                                    <div class="stat-item">
                                        <span class="stat-number">${this.requests.length}</span>
                                        <span class="stat-label">Total de Solicitações</span>
                                    </div>
                                    <div class="stat-item">
                                        <span class="stat-number">${this.requests.filter(r => r.status === 'pending').length}</span>
                                        <span class="stat-label">Pendentes</span>
                                    </div>
                                    <div class="stat-item">
                                        <span class="stat-number">${this.requests.filter(r => r.status === 'approved').length}</span>
                                        <span class="stat-label">Aprovadas</span>
                                    </div>
                                    <div class="stat-item">
                                        <span class="stat-number">${this.requests.filter(r => r.status === 'rejected').length}</span>
    ...
    addScheduleViewButton() {
        const dashboardGrid = document.querySelector('#coordenadorDashboard .dashboard-grid');
        if (dashboardGrid && !document.getElementById('viewScheduleBtn')) {
            ...
            document.getElementById('viewScheduleBtn').addEventListener('click', this.showScheduleView.bind(this));
        }
    }

    showScheduleView() {
        const approvedSchedules = this.requests.filter(req => req.status === 'approved');
                    ...
                    <div class="tab-content">
                        <div id="weekly-tab" class="tab-pane active">
                            <div class="weekly-schedule" id="weeklySchedule">
                                ${this.generateWeeklySchedule(approvedSchedules)}
                            </div>
                        </div>
                        
                        <div id="monthly-tab" class="tab-pane">
                            <div class="monthly-schedule" id="monthlySchedule">
                                ${this.generateMonthlySchedule(approvedSchedules)}
                            </div>
                        </div>
                        ...
                        <div id="list-tab" class="tab-pane">
                            <div class="schedule-list" id="scheduleListView">
                                ${approvedSchedules.length === 0 ? 
                                    ...
                                    approvedSchedules.map(schedule => this.createScheduleItem(schedule)).join('')
                                }
                            </div>
                        </div>
                    </div>
        ...
        // Event listeners
        document.getElementById('closeScheduleView').addEventListener('click', () => {
    ...
    generateWeeklySchedule(schedules) {
        ...
        return `
            <div class="weekly-grid">
                ${Object.entries(weekSchedules).map(([date, dayData]) => `
                    <div class="week-day">
                        <div class="day-header">
                            <h4>${weekDays[(dayData.date.getDay() + 6) % 7]}</h4>
                            <span class="day-date">${dayData.date.getDate()}/${dayData.date.getMonth() + 1}</span>
                        </div>
        ...
        const html = `
            ...
            <div class="monthly-grid">
                ${Array.from({ length: daysInMonth }, (_, i) => {
                    ...
                    return `
                        <div class="month-day ${isToday ? 'today' : ''}" data-day="${day}" data-month="${currentMonth}" data-year="${currentYear}" style="cursor: pointer;">
                            <div class="day-number">${day}</div>
                            <div class="day-schedules">
                                ${daySchedules.length === 0 ? '' : `
                                    <div class="schedule-count">${daySchedules.length} agendamento(s)</div>
                                    ...
                                    `).join('')}
                                    ${daySchedules.length > 2 ? `<div class="more-schedules">+${daySchedules.length - 2} mais</div>` : ''}
                                `}
                            </div>
                        </div>
                    `;
    ...
    showDayAgendamentosModal(day, month, year, daySchedules) {
        ...
        if (daySchedules.length === 0) {
            ...
            schedulesHTML = daySchedules.map(schedule => {
                const statusClass = schedule.status === 'approved' ? 'success' : 
                                   schedule.status === 'rejected' ? 'danger' : 'warning';
                const statusText = schedule.status === 'approved' ? 'APROVADO' :
                                  schedule.status === 'rejected' ? 'RECUSADO' : 'PENDENTE';
                ...
                return `
                    <div class="day-agendamento-item ${statusClass}">
                        <div class="agendamento-header">
                            ...
                            <span class="agendamento-status status-${schedule.status}">
                                ${statusText}
                            </span>
                        </div>
                        <div class="agendamento-content">
                            <h4 class="agendamento-subject">${schedule.subject}</h4>
                            ${schedule.message ? `<p class="agendamento-message">${schedule.message}</p>` : ''}
                            <div class="agendamento-details">
                                ${schedule.orientador ? `
                                    <p><strong>Orientador:</strong> ${schedule.orientador.name} - ${schedule.orientador.specialty}</p>
                                ` : ''}
                                ${schedule.userName ? `<p><strong>Solicitante:</strong> ${schedule.userName}</p>` : ''}
                                ${schedule.userEmail ? `<p><strong>Email:</strong> ${schedule.userEmail}</p>` : ''}
                                ${schedule.responseDate ? `<p><strong>Confirmado em:</strong> ${this.formatDate(schedule.responseDate)}</p>` : ''}
                            </div>
                        </div>
        ...
        document.getElementById('closeDayAgendamentos').addEventListener('click', () => {
    ...
    toggleDarkMode() {
        ...
        this.updateAccessibilityUI();
        this.showAccessibilityIndicator(isDark ? 'Modo Claro Ativado' : 'Modo Escuro Ativado');
    }
    ...
    increaseFontSize() {
        ...
        if (newSize !== currentSize) {
            ...
            this.updateAccessibilityUI();
            this.showAccessibilityIndicator(`Fonte: ${newSize.charAt(0).toUpperCase() + newSize.slice(1)}`);
        }
    }
    ...
    toggleHighContrast() {
        const isHighContrast = document.body.getAttribute('data-contrast') === 'high';
        document.body.setAttribute('data-contrast', isHighContrast ? 'normal' : 'high');
        this.saveAccessibilitySetting('contrast', isHighContrast ? 'normal' : 'high');
        this.updateAccessibilityUI();
        this.showAccessibilityIndicator(isHighContrast ? 'Alto Contraste Desativado' : 'Alto Contraste Ativado');
    }
    ...
    updateAccessibilityUI() {
        ...
        if (darkModeBtn) {
            const isDark = document.body.getAttribute('data-theme') === 'dark';
            darkModeBtn.innerHTML = `<i class="fas ${isDark ? 'fa-sun' : 'fa-moon'}"></i>`;
        ...
        if (highContrastBtn) {
            const isHighContrast = document.body.getAttribute('data-contrast') === 'high';
            highContrastBtn.innerHTML = `<i class="fas ${isHighContrast ? 'fa-adjust' : 'fa-adjust'}"></i>`;
            highContrastBtn.title = isHighContrast ? 'Contraste Normal' : 'Alto Contraste';
    ...
    showAccessibilityHelp() {
        ...
        document.getElementById('closeAccessibilityHelp').addEventListener('click', () => {
...
<<<<<<< HEAD
                ...
                if (this.userType === 'coordenador') {
                    genericOrientadorQuery = await db.collection('solicitacoes')
...
>>>>>>> 263b65695a8b407399dac2ee412504ba2bf68fa5
    ...
    generateTimeSlotsHTML() {
        ...
        return timeSlots.map(slot => `
            <div class="time-slot-item">
                <div class="slot-info">
                    <span class="slot-day">${this.getDayName(slot.dayOfWeek)}</span>
                    <span class="slot-time">${slot.startTime} - ${slot.endTime}</span>
                    <span class="slot-duration">${slot.duration} min</span>
                </div>
                <div class="slot-actions">
                    <button class="btn btn-sm btn-outline" onclick="agendaSystem.editTimeSlot('${slot.id}')">
                        <i class="fas fa-edit"></i>
                    </button>
                    <button class="btn btn-sm btn-danger" onclick="agendaSystem.deleteTimeSlot('${slot.id}')">
    ...
    generateBlockedDatesHTML() {
        ...
        return blockedDates.map(date => `
            <div class="blocked-date-item">
                <div class="date-info">
                    <span class="blocked-date">${this.formatDate(date.date)}</span>
                    <span class="blocked-reason">${date.reason}</span>
                </div>
                <div class="date-actions">
                    <button class="btn btn-sm btn-outline" onclick="agendaSystem.editBlockedDate('${date.id}')">
                        <i class="fas fa-edit"></i>
                    </button>
                    <button class="btn btn-sm btn-danger" onclick="agendaSystem.deleteBlockedDate('${date.id}')">
                        <i class="fas fa-trash"></i>
                    </button>
                </div>
            </div>
    ...
    createScheduleItem(schedule) {
        ...
        return `
            <div class="schedule-item ${statusClass} ${isUpcoming ? 'upcoming' : 'past'}">
    ...
    showAddBlockedDateForm() {
        ...
        document.getElementById('addBlockedDateForm').addEventListener('submit', (e) => {
            e.preventDefault();
    ...
    saveTimeSlot() {
        ...
        if (startTime >= endTime) {
            this.showNotification('O horário de início deve ser anterior ao horário de fim.', 'error');
            return;
        }
        ...
        selectedDays.forEach(dayValue => {
            ...
            if (conflictingSlot) {
                this.showNotification(`Já existe um horário configurado para ${this.getDayName(dayOfWeek)} neste período.`, 'error');
                return;
            }
        ...
        const daysText = selectedDays.map(day => this.getDayName(parseInt(day))).join(', ');
        this.showNotification(`Horário adicionado com sucesso para: ${daysText}`, 'success');
        this.refreshTimeSlots();
    }

    saveBlockedDate() {
        const formData = new FormData(document.getElementById('addBlockedDateForm'));
        const blockedDate = {
            id: Date.now().toString(),
    ...
    saveScheduleSettings() {
        const formData = new FormData(document.getElementById('scheduleSettingsForm'));
    ...
    refreshTimeSlots() {
        document.getElementById('timeSlotsContainer').innerHTML = this.generateTimeSlotsHTML();
    }

    refreshBlockedDates() {
        document.getElementById('blockedDatesContainer').innerHTML = this.generateBlockedDatesHTML();
    }
    ...
    deleteTimeSlot(slotId) {
        if (confirm('Tem certeza que deseja excluir este horário?')) {
            const timeSlots = this.getTimeSlots().filter(slot => slot.id !== slotId);
            localStorage.setItem('timeSlots', JSON.stringify(timeSlots));
    ...
    deleteBlockedDate(dateId) {
        if (confirm('Tem certeza que deseja excluir esta data bloqueada?')) {
            const blockedDates = this.getBlockedDates().filter(date => date.id !== dateId);
            localStorage.setItem('blockedDates', JSON.stringify(blockedDates));
                ...
                <div class="modal-content">
                    ...
                    <form id="editScheduleForm" class="auth-form">
                        <div class="form-group">
                            ...
                            <input type="date" id="editScheduleDate" name="date" value="${schedule.date}" required>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="editScheduleSubject" name="subject" value="${schedule.subject}" required>
                        </div>
                        <div class="form-group">
                            <label for="editScheduleMessage">Mensagem</label>
                            <textarea id="editScheduleMessage" name="message" rows="4">${schedule.message || ''}</textarea>
                        </div>
                        <div class="form-actions">
                            <button type="submit" class="btn btn-primary">
        ...
        return recentRequests.map(request => `
            <div class="timeline-item">
                ...
                <div class="timeline-content">
                    <h4>${request.subject}</h4>
                    <p>${request.userName} - ${this.formatDate(request.date)}</p>
                    <span class="timeline-status status-${request.status}">${this.getStatusText(request.status)}</span>
                </div>
            </div>
        `).join('');
    }
    ...
    generateTopUsers() {
        ...
        return topUsers.map(([userName, count]) => `
            <div class="user-item">
    ...
    exportReport() {
        ...
        link.href = url;
        link.download = `relatorio-agendamentos-${new Date().toISOString().split('T')[0]}.json`;
        link.click();
            ...
            <html>
                ...
                <body>
                    ...
                    <div class="stats">
                        ...
                        <div class="stat">
                            <div class="stat-number">${this.requests.filter(r => r.status === 'approved').length}</div>
                            <div>Aprovadas</div>
                        </div>
                        <div class="stat">
                            <div class="stat-number">${this.requests.filter(r => r.status === 'rejected').length}</div>
                            <div>Rejeitadas</div>
                        </div>
                    </div>
